...
snd::analysis_tools::VetoPlane::VetoPlane(std::vector<MuFilterHit*> snd_hits, const Configuration &configuration, MuFilter *muon_filter_geometry, int station) : configuration_(configuration), station_(station)
{
...
const double snd::analysis_tools::VetoPlane::GetTotQdc() const
{
...
const double snd::analysis_tools::VetoPlane::GetBarQdc(int bar_to_compute) const
{
    double bar_qdc = std::accumulate(hits_.begin(), hits_.end(), 0.0,
                                [bar_to_compute](double sum, const auto &b) {return (b.bar == bar_to_compute) ? sum + b.qdc : sum;});
    return bar_qdc;
}

const int snd::analysis_tools::VetoPlane::GetBarNHits(int bar_to_compute) const
{
...
void snd::analysis_tools::VetoPlane::TimeFilter(double min_timestamp, double max_timestamp)
{
...
const int snd::analysis_tools::VetoPlane::GetNHitBars() const
{
    int count{0};
    for (int bar{0}; bar < configuration_.veto_bar_per_station; ++bar) {
        if (GetBarNHits(bar) > configuration_.veto_min_hit_on_bar) count++;
    }
    return count;
}

void snd::analysis_tools::VetoPlane::VetoHit::Print() const 
{
    LOGF(INFO, "VetoHit ch_idx :%d\tposition: (%f,%f,%f)\ttime: %f\tqdc: %f\tbar: %d\tis_x: %d\tis_right: %d", channel_index, x, y, z, timestamp, qdc, bar, is_x, is_right);
}