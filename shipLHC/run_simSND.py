...
parser.add_argument("--eMin", type=float, help="energy cut", dest="ecut", default=-1.0)
parser.add_argument("--zMax", type=float, help="max distance to apply energy cut", dest="zmax", default=70000.0,)
...
parser.add_argument("--enhancePiKaDecay", dest="enhancePiKaDecay", help="decrease charged pion and kaon lifetime", required=False, type=float, default=0.0,)
...
if options.testbeam:
    ...
    snd_geo = ConfigRegistry.loadpy(
        "$SNDSW_ROOT/geometry/sndLHC_HXgeom_config.py", tb_2023_mc=options.testbeam2023
    )
...
if simEngine == "PG":
    ...
    if options.multiplePGSources:
        targetZpos = np.array(
            np.arange(options.nZSlices) * options.zSliceStep * u.cm, dtype="d"
        )
        primGen.SetMultTarget(
            len(targetZpos), targetZpos, 0 * u.cm
        )  # dummy thickness set to 0
    print(
    ...
    ROOT.FairLogger.GetLogger().SetLogScreenLevel(
        "WARNING"
    )  # otherwise stupid printout for each event
# -----muon DIS Background------------------------
if simEngine == "muonDIS":
    ...
    mu_start, mu_end = (
        -3.7 - 2.0
    ) * u.m, -0.3 * u.m  # tunnel wall -30cm in front of SND
    ...
    inactivateMuonProcesses = (
        True  # avoid unwanted hadronic events of "incoming" muon flying backward
    )
    run.SetPythiaDecayer("DecayConfigPy8.C")
...
if simEngine == "Nuage":
    ...
    Nuagegen.SetPositions(
        0.0,
        -snd_geo.EmulsionDet.zdim / 2,
        snd_geo.EmulsionDet.zdim / 2,
        -snd_geo.EmulsionDet.xdim / 2,
        snd_geo.EmulsionDet.xdim / 2,
        -snd_geo.EmulsionDet.ydim / 2,
        snd_geo.EmulsionDet.ydim / 2,
    )
    ut.checkFileExists(inputFile)
...
if simEngine == "Genie":
    ...
    neutrino_vtx_start_z = (
        snd_geo.MuFilter.Veto1Dy - snd_geo.MuFilter.VetoBarZ / 2.0 - tolerance_vtx_z
    )
    # To last Scifi plane
    neutrino_vtx_end_z = (
        snd_geo.Scifi.Ypos4 + snd_geo.Scifi.zdim / 2.0 + tolerance_vtx_z
    )

    Geniegen.SetPositions(-480 * u.m, neutrino_vtx_start_z, neutrino_vtx_end_z)

    Geniegen.SetDeltaE_Matching_FLUKAGenie(
        10.0
    )  # energy range for the search of a GENIE interaction with similar energy of FLUKA neutrino
...
if simEngine == "MuonBack":
    ...
    if fileType == "tree":
        ...
        primGen.SetTarget(snd_geo.target.z0 + 70.845 * u.m, 0.0)
    else:
        primGen.SetTarget(snd_geo.target.z0 + 50 * u.m, 0.0)
    #
...
if options.boostFactor > 1:
    ...
    gammaToMuPair = gProcessTable.FindProcess(
        ROOT.G4String("GammaToMuPair"), ROOT.G4String("gamma")
    )
    gammaToMuPair.SetCrossSecFactor(options.boostFactor)
    AnnihiToMuPair = gProcessTable.FindProcess(
        ROOT.G4String("AnnihiToMuPair"), ROOT.G4String("e+")
    )
    AnnihiToMuPair.SetCrossSecFactor(options.boostFactor)
    MuonToMuonPair = gProcessTable.FindProcess(
        ROOT.G4String("muToMuonPairProd"), ROOT.G4String("mu+")
    )
