...
parser.add_argument("-t", "--trackType", dest="trackType", help="DS or Scifi", default="Scifi")
...
def mergeTimeDiffs():
    ...
    for tag in ["", "X"]:
        for side in ["L", "R"]:
            M.h["dtBar_Veto" + side + tag] = M.h["dtBar_Veto4_4LX"].Clone(
                "dtBar_Veto" + side + tag
            )
            M.h["dtBar_Veto" + side + tag].Reset()
            for bar in range(nbars):
                for bar1 in [bar - 1, bar, bar + 1]:
                    if bar1 < 0 or bar1 > nbars - 1:
                        continue
...
if options.command == "TC":
    Tasks["vetoTimechannelcalibration"] = (
        vetoTimeCalibration.vetoTDCchannelCalibration()
    )
    Tasks["vetoTimeplanecalibration"] = vetoTimeCalibration.vetoTDCplaneCalibration()
    Tasks["vetoTimechannelcalibrationX"] = (
        vetoTimeCalibration.vetoTDCchannelCalibration()
    )
