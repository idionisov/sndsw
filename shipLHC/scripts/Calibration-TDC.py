...
def myPrint(tc, name, withRootFile=True):
    ...
    if withRootFile:
        tc.Print(name + "-run" + str(options.runNumber) + ".root")
...
if options.path.find("/eos") == 0:
    options.path = eosroot + options.path
...
def getTDCdelays(path="TDCdelays.txt"):
    ...
    for bar_number in range(systemAndBars[2]):
        for channel in range(8):
            h["D"].SetPoint(n, n, delays[10 * bar_number + channel] / 1000.0)
            n += 1
...
def getCalibrationParameters(runList=options.runNumbers):
    ...
    for r in runList.split(","):
        C[int(r)] = {1: {}, 2: {}}
        for s in [1, 2]:
            if int(r) < 100 and s == 1:
                continue  # no veto in H8
            tdcCanvas = (
                options.path + "TDC/TDCcalibration_" + sdict[s] + "-run" + r + ".root"
            )
...
def runStability(C=None):
    if not C:
        C = getCalibrationParameters(options.runNumbers)
        ...
        for s in C[r]:
            for l in C[r][s]:
                for i in C[r][s][l]["mean"]:
                    ...
                    if rError > 0.05:
                        continue  # remove distributions with low statistics

                    if not key in runEvol[s]:
                        runEvol[s][key] = ROOT.TGraph()
                    n = runEvol[s][key].GetN()
                    runEvol[s][key].SetPoint(n, value[0], 1.0)

            ut.bookHist(h, "rms" + sdict[s], "rms", 1000, -1.0, 1.0)
            ...
            for key in runEvol[s]:
                ...
                print(
                    "%s : %5.2F  %5.2F  %5.2F"
                    % (key, x, runEvol[s][key].GetRMS(), runEvol[s][key].GetMean())
                )

            h["rms" + sdict[s] + "100"] = h["rms" + sdict[s]].Rebin(
                10, "rms" + sdict[s] + "100"
            )
    ...
    for r in X.split(","):
        ...
        for s in [1, 2]:
            ...
            for l in range(systemAndPlanes[s]):
                for bar in range(systemAndBars[s]):
                    for side in ["L", "R"]:
                        ...
                        for i in range(
                            systemAndChannels[s][1] + systemAndChannels[s][0]
                        ):
                            if i == t0_channel:
                                continue
            ...
            for l in range(systemAndPlanes[s]):
                    ...
                    for side in ["L", "R"]:
                        ...
                        for i in range(
                            ...
                            for x in ["sigmaTDC_" + tag, "TDCcalib_" + tag]:
                                if x.find("calib") > 0 and (i == t0_channel):
                                    continue

                                tc = h[x].cd(k)
                                opt = "same"

                                if i == 0 and x.find("calib") < 0:
                                    opt = ""
                                if i == 1 and x.find("calib") > 0:
                                    opt = ""
                ...
                ut.bookHist(
                    h, "TDCcalibMean_" + tag, ";SiPM channel ; [ns]", 160, 0.0, 160.0
                )
                ut.bookHist(
                    h, "TDCcalibSigma_" + tag, ";SiPM channel ; [ns]", 160, 0.0, 160.0
                )
            ...
            for l in range(systemAndPlanes[s]):
                ...
                for i in range(1, aHistS.GetNbinsX()):
                    if aHistS.GetBinContent(i) > 0:
                        h["gTDCcalibSigma_" + tag].SetPoint(
                            k, i - 1, aHistS.GetBinContent(i)
                        )
                        h["gTDCcalibSigma_" + tag].SetPointError(
                            k, 0.5, aHistS.GetBinError(i)
                        )
                        h["gTDCcalibMean_" + tag].SetPoint(
                            k, i - 1, aHistM.GetBinContent(i)
                        )
                        h["gTDCcalibMean_" + tag].SetPointError(
                            k, 0.5, aHistM.GetBinError(i)
                        )
                        k += 1
